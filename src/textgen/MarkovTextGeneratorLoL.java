...
public class MarkovTextGeneratorLoL implements MarkovTextGenerator {
	...
	public void train(String sourceText) {
		if (sourceText == null) {
			throw new NullPointerException("Source text is empty");
		}
		// Generate list of words from the input
		String regex = "[\\s]+";
		...
		if (words.length > 0) {
			...
			for (String word : words) {
				node = contains(prevWord);
				if (node == null) {
					node = new ListNode(prevWord.getWord());
					wordList.add(node);
				}
			...
			prevWord.addNextWord(this.starter);
		}
	}

	private ListNode contains(ListNode prevWord) {
		for (ListNode node : wordList) {
			if (node.getWord().equals(prevWord.getWord()))
				return node;
		}
		return null;
	}
	...
	public String generateText(int numWords) {
		...
		ListNode currWord = new ListNode(this.starter);
		String output = "";
		output = output.concat(this.starter);

		ListNode node;
		String randomWord;
		if (!this.wordList.isEmpty() || numWords == 0) {
			for (int i = 1; i < numWords; i++) {
				node = contains(currWord);
				randomWord = node.getRandomNextWord(this.rnGenerator);
				output = output.concat(" ").concat(randomWord);
				currWord = new ListNode(randomWord);
			}
		}
		return output;
	}
